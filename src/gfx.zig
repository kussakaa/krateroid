...
pub const buffer = struct {
    ...
    pub fn data(id: Buffer.Id, bytes: []const u8) void {
    ...
    pub fn subdata(id: Buffer.Id, offset: usize, bytes: []const u8) void {
        gl.bindBuffer(@intFromEnum(buffers.items[id].target), _buffers_glid.items[id]);
        gl.bufferSubData(
            @intFromEnum(buffers.items[id].target),
            @intCast(offset),
...
pub const texture = struct {
    pub fn init(name: []const u8) !Texture.Id {
    ...
    pub fn use(id: Texture.Id) void {
        gl.bindTexture(gl.TEXTURE_2D, _textures_glid.items[id]);
    }
};

pub const program = struct {
    pub fn init(name: []const u8) !Program.Id {
        ...
        const vert = try shader.init(vert_path, .vert);
        ...
        const frag = try shader.init(frag_path, .frag);
...
pub const uniform = struct {
    pub fn init(id: Program.Id, name: []const u8) !Uniform.Id {
    ...
    pub fn set(id: Uniform.Id, value: anytype) void {
